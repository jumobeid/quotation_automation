    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-documents.update", [$crmDocument->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $crmDocument->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $crmDocument->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('requested_products') ? 'is-invalid' : '' }}" name="requested_products[]" id="requested_products" multiple required>
                    @foreach($requested_products as $id => $requested_products)
                        <option value="{{ $id }}" {{ (in_array($id, old('requested_products', [])) || $crmDocument->requested_products->contains($id)) ? 'selected' : '' }}>{{ $requested_products }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('quotation_request_date') ? 'is-invalid' : '' }}" type="text" name="quotation_request_date" id="quotation_request_date" value="{{ old('quotation_request_date', $crmDocument->quotation_request_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('department_name') ? 'is-invalid' : '' }}" type="text" name="department_name" id="department_name" value="{{ old('department_name', $crmDocument->department_name) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('quotation_is_signed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="quotation_is_signed" id="quotation_is_signed" value="1" {{ $crmDocument->quotation_is_signed || old('quotation_is_signed', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('quotation_is_new') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="quotation_is_new" id="quotation_is_new" value="1" {{ $crmDocument->quotation_is_new || old('quotation_is_new', 0) === 1 ? 'checked' : '' }} required>
