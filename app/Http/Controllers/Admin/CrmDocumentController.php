...
class CrmDocumentController extends Controller
{
    ...
    public function create()
    {
        ...
        $requested_products = RequestedProduct::all()->pluck('name', 'id');

        return view('admin.crmDocuments.create', compact('customers', 'requested_products'));
    }

    public function store(StoreCrmDocumentRequest $request)
    {
        $crmDocument = CrmDocument::create($request->all());
        $crmDocument->requested_products()->sync($request->input('requested_products', []));
    ...
    public function edit(CrmDocument $crmDocument)
    {
        ...
        $crmDocument->load('customer', 'requested_products');

        return view('admin.crmDocuments.edit', compact('customers', 'requested_products', 'crmDocument'));
    }
